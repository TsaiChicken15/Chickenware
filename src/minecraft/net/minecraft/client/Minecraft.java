...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    public void dispatchKeypresses()
    {
        ...
        {
        	if (this.currentScreen instanceof chickenware.ui.clickgui.ClickGui) {
                if (Keyboard.getEventKey() == 200 || Keyboard.getEventKey() == 208 || 
                	Keyboard.getEventKey() == 203 || Keyboard.getEventKey() == 205)
                	return; 
                if (Keyboard.getEventKey() == 14)
                	for (ArrayList<chickenware.ui.clickgui.button.ModuleButton> mbList : Client.ClickGui.modules) {
                		for (chickenware.ui.clickgui.button.ModuleButton m : mbList) {
                			if (m.listening) {
                				Client.ClickGui.playButtonPressSound();
                				m.m.setKey(0);
                				m.listening = false;
                			} 
                		} 	
                ...
                for (ArrayList<chickenware.ui.clickgui.button.ModuleButton> mbList : Client.ClickGui.modules) {
                	for (chickenware.ui.clickgui.button.ModuleButton m : mbList) {
                		if (m.listening) {
                			Client.ClickGui.playButtonPressSound();
                			m.m.setKey(Keyboard.getEventKey());
                			m.listening = false;
                		} 
                	} 
                } 
