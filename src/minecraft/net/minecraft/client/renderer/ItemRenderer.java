...
public class ItemRenderer
{
    ...
    public void func_178096_b(float p_178096_1_, float p_178096_2_)
    {
        ...
        {
        	if(Minecraft.getMinecraft().thePlayer.inventory.getCurrentItem() != null && Minecraft.getMinecraft().thePlayer.isUsingItem() &&
        			(Minecraft.getMinecraft().thePlayer.inventory.getCurrentItem().getItem() instanceof ItemPotion || Minecraft.getMinecraft().thePlayer.inventory.getCurrentItem().getItem() instanceof ItemFood))
        	{
        		GlStateManager.translate(ViewModel.x.get(), ViewModel.y.get(), -0.71999997F);
        	}
        	else
        	{
        		GlStateManager.translate(ViewModel.x.get(), ViewModel.y.get(), ViewModel.z.get());
        	}
        } 
        else 
        {
    ...
    public void renderItemInFirstPerson(float p_78440_1_)
    {
        ...
        if (this.itemToRender != null)
        {
            ...
            else if (var3.getItemInUseCount() > 0)
            {
                ...
                switch (ItemRenderer.SwitchEnumAction.field_178094_a[var7.ordinal()])
                {
                    ...
                    case 4:
                        //this.func_178096_b(var2, 0.0F);
                        //this.func_178103_d();
                    	Animations.renderAnimation(p_78440_1_);
                        break;
            ...
            {
            	if(Client.isEnabled("Killaura") != null)
            	{
            		if (Killaura.autoBlockValue.get() && CombatUtil.getEntity(Killaura.autoBlockRangeValue.get()) != null)
        			{
    ...
    private void renderFireInFirstPerson(float p_78442_1_)
    {
        ...
        for (int var5 = 0; var5 < 2; ++var5)
        {
            ...
            if (e.isCancelled()) 
            {
                GlStateManager.translate((-(var5 * 2 - 1) * 0.24F), LowFire.height.get(), 0.0D);
            } 
            else 
            {
