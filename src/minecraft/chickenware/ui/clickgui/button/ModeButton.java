...
public class ModeButton extends Button implements MCHook
{
    protected float w;
    protected float h;
    public float x;
    public float y;
    public ModeSetting m;

    public boolean hovered;

    public ModeButton(ModeSetting m, float x, float y, float widthIn, float heightIn)
    {
    	this.m= m;
        this.x = x;
        this.y = y;
    ...
    public void draw(int mouseX, int mouseY) {
    	...
    	mc.fontRendererObj.drawStringWithShadow("<", x + 5, (y + h / 2) - _fr.FONT_HEIGHT / 2, color);
    	mc.fontRendererObj.drawStringWithShadow(">", x + w - mc.fontRendererObj.getStringWidth(">") - 5, (y + h / 2) - _fr.FONT_HEIGHT / 2, color);
    	mc.fontRendererObj.drawStringWithShadow(m.name + ": " + m.get(), (x + x + w + 4) / 2 - mc.fontRendererObj.getStringWidth(m.name + ": " + m.get()) / 2, (y + h / 2) - _fr.FONT_HEIGHT / 2, color);
    }
}